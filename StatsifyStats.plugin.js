...
module.exports = (_ => {
	...
	return (window.Lightcord || window.LightCord) ? class {
		...
		// _ Actual plugin code starts here

		// ! Just a naming thing but "player" and "player2" might sound a bit confusing
		// ! Maybe make it hypixelPlayer and hyApiPlayer or something (if ofc u care about clarity for others :)
      ...
      <div id="bodyResult" class="content-3YMskv">
		...
		return class StatsifyStats extends Plugin {
			...
			onLoad() {
				...
				this.patchedModules = {
					after: {
						// ! Does this have to be here? Again empty but maybe its needed?
					}
				};
			}
			...
			forceUpdateAll() {
				// ! Does this have to be here? Looks like it can be removed because its empty
				// ! It does get called twice so im not sure
			}
			...
			// ! For the code from "getRank" to "getRankColor" why not use the HyAPI request you already made? 
			// ! It includes the Hex code you define here as well as a plus colour and the rank colour
			getRank = player => {
				let rank = 'NON';
			...
			status() {
				...
				status.src = player2.online ? "https://statsify.net/img/assets/online.png" : "https://statsify.net/img/assets/offline.png";
			}

			loadProfile() {
				// Can't this be a for of loop? 
				// Maybe a map that holds the images?
			...
			popoverUpdater() {
				...
				let gcolor = "";
				let networkLevel = ((Math.sqrt(player.networkExp + 15312.5) - 125 / Math.sqrt(2)) / (25 * Math.sqrt(2))).toFixed(0);
				let rawgcolor = `${guild.guild ? guild.guild.tag ? ` ${body_guild_mcColor.mc}[${guild.guild.tag}${body_guild_mcColor.mc}]` : "" : ""}`
